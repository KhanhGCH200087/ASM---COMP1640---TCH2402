...
router.put('/editST/:id', verifyToken, upload.single('image'), async (req, res) => {
    const studentID = req.params.id;
    const studentData = await StudentModel.findById(studentID);
    if (!studentData) {
        res.status(404).json({ success: false, error: "Student not found" });
...
router.post('/submitContribution/:id', verifyToken, upload.single('image'), async (req, res) => {
    try {
        const userId = req.userId;
        const userData = await UserModel.findById(userId);
        if(!userData){
...
router.put('/editContribution/:id', verifyToken, upload.single('contribution'), async(req, res) => {
    try {
        const userId = req.userId;
        const userData = await UserModel.findById(userId);
        if(!userData){
