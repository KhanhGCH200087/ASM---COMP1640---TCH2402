...
router.put('/edit/:id', verifyToken, checkAdminSession, upload.single('image'), async (req, res) => {
    try {
        // Fetch student by ID
        const studentId = req.params.id;
        const student = await StudentModel.findById(studentId);
...
router.get('/profile', verifyToken, checkStudentSession, async (req, res) => {
    try{
        var stUserId = req.session.user_id;
        var UserData = await UserModel.findById(stUserId);
      if(UserData){
...
router.get('/editST/:id', verifyToken, checkStudentSession, async (req, res) => {
    const studentId = req.params.id;
...
router.put('/editST/:id', verifyToken, checkStudentSession, upload.single('image'), async (req, res) => {
    const studentID = req.params.id;
    const studentData = await StudentModel.findById(studentID);
    if (!studentData) {
        res.status(404).json({ success: false, error: "Student not found" });
...
router.get('/facultypage', verifyToken, checkStudentSession, async(req, res) => {
    try{
        var stUserId = req.session.user_id;
        var UserData = await UserModel.findById(stUserId);
        var stID = req.session.st_id;
...
//------------------nộp bài trong event ----------------------------------------------
router.get('/submitContribution/:id', verifyToken, checkStudentSession, async (req, res) => {
    try {
...
router.post('/submitContribution/:id', verifyToken, checkStudentSession, upload.single('image'), async (req, res) => {
    try {
        const eventId = req.params.id;
        const eventData = await EventModel.findById(eventId);
        const eventFacultyID = eventData.faculty;
...
router.get('/eventDetail/:id', verifyToken, checkStudentSession, async (req, res) => { 
    try{
...
router.delete('/deleteContribution/:id', verifyToken, checkStudentSession, async(req, res) => {
    try{
...
router.get('/editContribution/:id', verifyToken, checkStudentSession, async(req, res) => {
    try {
        // Fetch contribution details by ID
        const contributionId = req.params.id;
...
router.put('/editContribution/:id', verifyToken, checkStudentSession, upload.single('contribution'), async(req, res) => {
    try {
        const contributionId = req.params.id;
        const contribution = await ContributionModel.findById(contributionId).populate('event');
        if (!contribution) {
