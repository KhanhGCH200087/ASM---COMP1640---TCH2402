...
router.get('/profile',verifyToken, checkMCSession, async (req, res) => {
    try{
...
router.get('/editMC/:id',verifyToken, checkMCSession, async (req, res) => {
    const marketingcoordinatorId = req.params.id;
    const marketingcoordinator = await MarketingCoordinatorModel.findById(marketingcoordinatorId);
    if (!marketingcoordinator) {
        res.status(404).json({ success: false, error: "Marketing Coordinator not found" });
...
router.put('/editMC/:id',verifyToken, checkMCSession, upload.single('image'), async (req, res) => {
    const marketingcoordinatorId = req.params.id;
    const marketingcoordinator = await MarketingCoordinatorModel.findById(marketingcoordinatorId);
    if (!marketingcoordinator) {
        res.status(404).json({ success: false, error: "Marketing Coordinator not found" });
...
router.get('/facultypage',verifyToken, checkMCSession, async(req, res) => {
    try{
        var mcUserId = req.session.user_id;
        var UserData = await UserModel.findById(mcUserId);
        var mcID = req.session.mc_id;
...
router.get('/eventDetail/:id',verifyToken, checkMCSession, async (req, res) => {  
    try{
...
router.get('/contributionDetail/:id',verifyToken, checkMCSession, async(req, res) => {
    try {
...
router.put('/contributionDetail/:id',verifyToken, checkMCSession, async(req, res) => {
    try {
